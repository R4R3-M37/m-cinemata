...
import { baseUrl, categoryFormatter, currentYear, sortByFormatter, token } from '~/shared/api/config'
...
export const movieByIDQuery = createQuery({
	effect: createEffect(async ({ id }: { id: string }): Promise<Movies> => {
		const response = await fetch(`${baseUrl}/movie?search=${id}&field=id${token}`)
...
export const moviesByNameQuery = createQuery({
	effect: createEffect(async ({ name, page }: { name: string; page: number }): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=name&search=${name}&field=poster.url&search=!null&sortField=year&sortType=-1&isStrict=false&page=${page}${token}`
		)
...
export const moviesByGenreQuery = createQuery({
	effect: createEffect(async ({ genre, page }: { genre: string; page: number }): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=${genre}&sortField=rating.kp&field=name&search=!null&field=poster.url&search=!null&sortType=-1&page=${page}${token}`
		)
...
export const bestHorrorsQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=ужасы&field=rating.kp&search=7-10&field=name&search=!null&field=poster.url&search=!null&sortField=votes.kp&sortType=-1&page=1&limit=20${token}`
		)
...
export const bestCountOfVotesQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(`${baseUrl}/movie?sortField=votes.kp&sortType=-1&page=1&limit=20${token}`)
...
export const sciFiSerialsQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=%D1%84%D0%B0%D0%BD%D1%82%D0%B0%D1%81%D1%82%D0%B8%D0%BA%D0%B0&field=typeNumber&search=2&sortField=votes.kp&sortType=-1&sortField=year&field=name&search=!null&field=poster.url&search=!null&sortType=-1&page=1&limit=20${token}`
		)
...
export const comedyAnimeQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=комедия&field=typeNumber&search=4&sortField=votes.kp&sortType=-1&sortField=year&field=name&search=!null&field=poster.url&search=!null&sortType=-1&page=1&limit=20${token}`
		)
...
export const bestComedyQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=комедия&field=rating.kp&search=7-10&sortField=votes.kp&sortType=-1&page=1&limit=20${token}`
		)
...
export const horrorsAnimeQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=ужасы&field=typeNumber&search=4&sortField=rating.kp&sortType=-1&page=1&limit=20${token}`
		)
...
export const newDetectivesQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(
			`${baseUrl}/movie?field=genres.name&search=детектив&field=rating.kp&search=7-10&sortField=year&sortType=-1&page=1&limit=20${token}`
		)
...
export const top250MoviesQuery = createQuery({
	effect: createEffect(async ({ page }: { page: number }): Promise<MoviesRoot> => {
		const response = await fetch(`${baseUrl}/movie/?field=top250&search=!null&page=${page}${token}`)
...
export const top10MoviesQuery = createQuery({
	effect: createEffect(async (): Promise<MoviesRoot> => {
		const response = await fetch(`${baseUrl}/movie/?field=top10&search=!null${token}`)
...
export const personByIDQuery = createQuery({
	effect: createEffect(async ({ id }: { id: string }) => {
		const response = await fetch(`${baseUrl}/person?field=id&search=${id}${token}`)
